...
class ResNet18WithSSMCTB(nn.Module):
    def __init__(self, num_classes=1000, use_pretrained=True):
        super(ResNet18WithSSMCTB, self).__init__()
        self.resnet = resnet18(pretrained=use_pretrained)
        self.ssmctb = SSMCTB(channels=512, kernel_dim=3, dilation=1)  # Adjust parameters as needed
    ...
    def forward(self, x):
        ...
        # Integrate SSMCTB after the last residual layer
        x, _ = self.ssmctb(x)
